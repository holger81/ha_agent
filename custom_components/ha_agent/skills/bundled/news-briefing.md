...
slots:
  - name: digest_scope
    description: Optional focus for the briefing (local, germany, global/today, etc.)
    default: ""
tool_steps:
...
1. Call `mcp_news__news_curate` with `digest_scope` from `{{digest_scope}}` when the user named a focus (e.g. local, germany). Omit `digest_scope` for a general/global briefing — do not invent a query parameter.
